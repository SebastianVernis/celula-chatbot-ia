...
- Ajustar 'sizes' según el diseño real de tu sitio
- Cambiar 'width' y 'height' con las dimensiones reales para evitar CLS
-->
EOF
    
    print_success "  Template HTML generado: ${name}.html"
}

# Generar informe de optimización
generate_report() {
    print_header "Generando informe de optimización"
    
    local report_file="${OUTPUT_DIR}/optimization-report.txt"
...
WebP:
$(find "$OUTPUT_DIR/webp" -type f -name "*.webp" | wc -l) archivos
Tamaño total: $(du -sh "$OUTPUT_DIR/webp" 2>/dev/null | cut -f1 || echo "0")

AVIF:
$(find "$OUTPUT_DIR/avif" -type f -name "*.avif" | wc -l) archivos
Tamaño total: $(du -sh "$OUTPUT_DIR/avif" 2>/dev/null | cut -f1 || echo "0")

Templates HTML:
$(find "$OUTPUT_DIR/html-templates" -type f -name "*.html" | wc -l) archivos

────────────────────────────────────────────────────────
COMPARACIÓN DE TAMAÑOS PROMEDIO:
────────────────────────────────────────────────────────

WebP: Reducción típica del 25-35% vs JPG
AVIF: Reducción típica del 40-50% vs JPG

════════════════════════════════════════════════════════
EOF
    
    cat "$report_file"
    print_success "Informe guardado en: $report_file"
}
...
main() {
    print_header "Optimización de Imágenes Responsivas"
    
    echo "Configuración:"
    echo "  Entrada: $INPUT_DIR"
    echo "  Salida: $OUTPUT_DIR"
    echo "  Tamaños: $SIZES"
    echo "  Calidad WebP: $QUALITY_WEBP"
    echo "  Calidad AVIF: $QUALITY_AVIF"
    echo ""
    ...
    print_header "Procesando imágenes"
    ...
    while IFS= read -r -d '' file; do
        process_image "$file"
        ((image_count++))
    done < <(find "$INPUT_DIR" -maxdepth 1 -type f -iname "*.png" -print0)
    
    if [ $image_count -eq 0 ]; then
        print_error "No se encontraron imágenes JPG o PNG en $INPUT_DIR"
        exit 1
    fi
    ...
    print_success "$image_count imágenes procesadas"
    print_info "Los templates HTML están en: ${OUTPUT_DIR}/html-templates/"
}
