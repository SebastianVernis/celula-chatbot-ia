...
class CelulaChatbotManager {
  ...
  async handleFormSubmission() {
    ...
    if (this.leadData.name && this.leadData.email && this.leadData.phone) {
      // Enviar lead capturado con Resend
      if (window.ResendEmailHandler) {
        await window.ResendEmailHandler.sendChatbotLead(this.leadData);
      }
...
Tipo de evento: ${this.leadData.eventType || "[Sin especificar]"}`;
  ...
  async sendConversationSummary() {
    try {
      ...
      // Usar ResendEmailHandler
      if (!window.ResendEmailHandler) {
        console.error("ResendEmailHandler no disponible");
        return false;
      }
      ...
      const result = await window.ResendEmailHandler.sendChatbotSummary(
        this.leadData,
        conversationData
      );
  ...
  async handleUserInput() {
    ...
    if (this.shouldSendSummary() && !this.emailSent) {
      console.log('🔔 Condiciones cumplidas para enviar resumen del chatbot');
      console.log('📊 Estado:', {
        shouldSend: this.shouldSendSummary(),
        emailSent: this.emailSent,
      ...
      setTimeout(() => {
        ...
        this.saveState(); // Guardar el estado actualizado
      }, 2000);
    } else {
      console.log('ℹ️ No se envía resumen. Estado:', {
        shouldSend: this.shouldSendSummary(),
        emailSent: this.emailSent,
