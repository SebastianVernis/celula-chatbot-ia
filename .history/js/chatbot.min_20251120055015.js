class CelulaChatbotManager {
  constructor() {
    (console.log("🔧 Construyendo CelulaChatbotManager..."),
      ...
      (this.chatWindowContainer = document.getElementById(
    ...
    const e = {
      ...
      a = Object.entries(e)
        ...
        .map(([e]) => e);
    (a.length > 0
      ? console.error("❌ Elementos faltantes del chatbot:", a)
      : console.log("✅ Todos los elementos del chatbot encontrados"),
      this.init());
  }
  async init() {
    (this.setupEventListeners(), this.loadState());
  }
  saveState() {
    const e = {
    ...
    sessionStorage.setItem("celulaChatbotState", JSON.stringify(e));
  }
  loadState() {
    const e = sessionStorage.getItem("celulaChatbotState");
    if (e)
      try {
        const a = JSON.parse(e);
        ((this.chatHistory = a.chatHistory || []),
          (this.leadData = a.leadData || {}),
          Object.keys(this.leadData).length > 0 && this.fillLeadForm());
      } catch (e) {
        (console.error("Error al cargar el estado del chatbot:", e),
          this.resetState());
      }
  ...
  isVisibleMessage(e) {
    return (
      !(
        e.parts[0].text.includes("Eres el Asistente Musical Virtual") ||
        e.parts[0].text.includes("MISIÓN PRINCIPAL:") ||
        e.parts[0].text.includes("MÉTODO SPIN") ||
        e.parts[0].text.includes("DIRECTRICES CRÍTICAS:")
      ) &&
      ("model" !== e.role ||
        !e.parts[0].text.includes(
          "¡Entendido! Soy el Asistente Musical de Grupo Musical Versátil La Célula"
        ))
    );
  }
  getVisibleMessages() {
    return this.chatHistory.filter((e) => this.isVisibleMessage(e));
  }
  repopulateChat() {
    ((this.chatWindow.innerHTML = ""),
      this.getVisibleMessages().forEach((e) => {
        "user" === e.role
          ? this.appendMessage(e.parts[0].text, "user")
          : "model" === e.role && this.appendMessage(e.parts[0].text, "bot");
      }));
  }
  setupEventListeners() {
    ...
    (e
      ? (console.log("✅ Botón chatbot-toggle encontrado, agregando listener"),
        e.addEventListener("click", () => {
          (console.log("🖱️ Click en chatbot-toggle detectado"),
            ...
            this.chatHistory && this.chatHistory.length > 3
              ...
              : this.leadData && Object.keys(this.leadData).length > 0
      ...
      document
        ...
        ?.addEventListener("click", () => {
          (this.leadForm.classList.remove("active"), this.saveState());
        }),
      document.getElementById("chat-close")?.addEventListener("click", () => {
        (this.chatWindowContainer.classList.remove("active"), this.saveState());
      }));
    const a = document.createElement("button");
    ((a.id = "reset-chat"),
      (a.className = "reset-chat"),
      a.setAttribute("aria-label", "Borrar conversación"),
      (a.innerHTML = "🗑️"),
      (a.title = "Borrar esta conversación y comenzar de nuevo"),
      (a.style.cssText =
        "position: absolute; right: 40px; top: 15px; background: transparent; border: none; color: white; cursor: pointer; font-size: 16px;"));
    const t = document.querySelector(".chat-header");
    (t && t.appendChild(a),
      ...
      this.closeBtn?.addEventListener("click", () => {
        (parent.postMessage("close-chatbot", "*"), this.saveState());
      }),
      this.sendBtn?.addEventListener("click", () => this.handleUserInput()),
      this.userInput?.addEventListener("keypress", (e) => {
        "Enter" !== e.key ||
          e.shiftKey ||
          (e.preventDefault(), this.handleUserInput());
      }),
      ...
      setInterval(() => {
        this.chatHistory.length > 0 && this.saveState();
      }, 3e4));
  }
  autoResize(e) {
    const a = e.target;
  ...
  async handleFormSubmission() {
    const e = document.getElementById("name-input"),
      a = document.getElementById("email-input"),
      t = document.getElementById("phone-input"),
      n = document.getElementById("event-type-input");
    ((this.leadData.name = e.value.trim()),
      (this.leadData.email = a.value.trim()),
      (this.leadData.phone = t.value.trim()),
      (this.leadData.eventType = n.value.trim()),
      this.leadData.name &&
        this.leadData.email &&
        this.leadData.phone &&
        (this.leadForm.classList.remove("active"),
  ...
  async loadInitialContext() {
    try {
      ...
      return !this.chatHistory.some(
        (e) =>
          "model" === e.role &&
          e.parts[0].text.includes("¡Hola") &&
          e.parts[0].text.includes("Soy el **Asistente Musical**")
      );
    } catch (e) {
      return (
        console.error(e),
  ...
  async getBotResponse(e) {
    this.chatHistory.push({ role: "user", parts: [{ text: e }] });
    const a = { history: this.chatHistory };
    try {
      const e = await fetch("/api/chatbot", {
        method: "POST",
      ...
      return (
        this.chatHistory.push({ role: "model", parts: [{ text: n }] }),
        n
      );
    } catch (e) {
      return (
        console.error("Error:", e.message),
        "Lo siento, ocurrió un error al procesar tu mensaje. Para atención inmediata, contáctanos por WhatsApp al 55 3541 2631."
      );
    }
  }
  appendMessage(e, a) {
    ...
    if (
      (t.classList.add(
        "message",
        "user" === a ? "user-message" : "bot-message"
      ),
      "bot" === a)
    ) {
      const a = this.processMarkdown(e);
      t.innerHTML = a;
    } else t.textContent = e;
    (this.chatWindow.appendChild(t), this.scrollToBottom());
  }
  processMarkdown(e) {
    let a = e.replace(/\n\n/g, "</p><p>");
    ((a = "<p>" + a + "</p>"),
      (a = a.replace(/\n/g, "<br>")),
      (a = a.replace(/<p><\/p>/g, "")),
      (a = a.replace(/<p><br><\/p>/g, "")),
      (a = a.replace(/\*\*(.*?)\*\*/g, "<strong>$1</strong>")),
      (a = a.replace(/\*([^*<>]+?)\*/g, function (e, a) {
        return "<em>" + a + "</em>";
      })),
      (a = a.replace(/<p>[-*•]\s+(.+?)(<br>|<\/p>)/g, "<p><li>$1</li>$2")),
      (a = a.replace(/<br>[-*•]\s+(.+?)(<br>|<\/p>)/g, "<br><li>$1</li>$2")),
      (a = a.replace(
        /(<li>.*?<\/li>)(\s*<br>\s*<li>.*?<\/li>)*/gs,
        "<ul>$&</ul>"
      )),
      (a = a.replace(/<p>\s*<\/p>/g, "")),
      (a = a.replace(/(<\/p>)\s*(<p>)/g, "$1$2")),
      (a = a.replace(
        /(https?:\/\/[^\s<>]+)/g,
    ...
    return (
      ...
      a.includes("🎵") ||
        (a = a.replace(/\b(música|musical|músicos)\b/gi, "🎵 $1")),
      a.includes("💍") || (a = a.replace(/\b(boda|bodas)\b/gi, "💍 $1")),
      a.includes("🎓") ||
        (a = a.replace(/\b(graduación|graduaciones)\b/gi, "🎓 $1")),
      a.includes("🎉") ||
        (a = a.replace(/\b(fiesta|fiestas|celebración|evento)\b/gi, "🎉 $1")),
      a
    );
  }
  async sendConversationSummary() {
    try {
      const e = this.chatHistory
          .filter((e) => "user" === e.role)
          .map((e) => e.parts[0].text)
          .filter(
            (e) => e.length > 10 && !e.includes("Eres el Asistente Musical")
          ),
        a = this.chatHistory
          .filter((e) => "model" === e.role)
          .map((e) => e.parts[0].text);
      if (e.length < 2)
        return (console.log("Conversación muy corta, no se enviará email"), !1);
      const t = this.chatHistory
          .filter((e) => !e.parts[0].text.includes("Eres el Asistente Musical"))
          .map(
            (e) =>
              `${"user" === e.role ? "👤 Cliente" : "🤖 Asistente"}: ${e.parts[0].text}`
          )
          .join("\n\n"),
        n = {
          type: "chatbot",
      ...
      return i.success
        ? (console.log("✅ Resumen de conversación enviado por email"),
          ...
          !0)
        : (console.error("❌ Error enviando email:", i.error),
          i.errors && console.error("Errores de validación:", i.errors),
          !1);
    } catch (e) {
      return (console.error("Error al enviar resumen de conversación:", e), !1);
    }
  }
  showEmailSentNotification() {
    const e = document.createElement("div");
    ((e.className = "email-notification"),
      (e.innerHTML =
  ...
  shouldSendSummary() {
    const e = this.chatHistory
      ...
      t = e.some((e) => a.some((a) => e.toLowerCase().includes(a)));
    return e.length >= 3 || (e.length >= 2 && t);
  }
  scrollToBottom() {
  ...
  showTypingIndicator() {
    const e = document.createElement("div");
    (e.classList.add("message", "bot-message", "typing-indicator"),
      (e.innerHTML = "<span>Componiendo respuesta...</span>"),
  ...
  async handleUserInput() {
    const e = this.userInput.value.trim();
    if (e && !this.isLoading) {
      ((this.isLoading = !0),
        (this.sendBtn.disabled = !0),
        this.appendMessage(e, "user"),
        (this.userInput.value = ""),
        this.showTypingIndicator());
      try {
        const a = await this.getBotResponse(e);
        (this.removeTypingIndicator(), this.appendMessage(a, "bot"));
      } catch (e) {
      ...
      ((this.isLoading = !1),
        ...
        this.saveState(),
        this.shouldSendSummary() &&
          !this.emailSent &&
          setTimeout(() => {
            (this.sendConversationSummary(), (this.emailSent = !0));
          }, 2e3));
    }
  }
}
function initializeChatbot() {
  ...
  try {
    const e = new CelulaChatbotManager(),
      a = document.getElementById("chatbot-toggle"),
      t = document.getElementById("lead-form"),
      n = document.getElementById("chat-window-container"),
    ...
    ((s.textContent =
      ...
      (window.celulaChatbotManager = e));
  } catch (e) {
    console.error("❌ Error al inicializar el chatbot:", e);
  }
}
