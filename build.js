...
// Copy root files
console.log('\n📋 Copying root files...');
['robots.txt', 'sitemap.xml', 'manifest.json', '_headers', 'sw.js'].forEach(file => {
...
// Copy main HTML files to root of dist
['index.html', 'blog.html', 'cotizador.html', 'offline.html'].forEach(file => {
  const srcPath = join(DIST_DIR, 'html', file);
  if (existsSync(srcPath)) {
    cpSync(srcPath, join(DIST_DIR, file));
